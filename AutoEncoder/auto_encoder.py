...
class AdditiveGaussianNoiseAutoencoder(object):
    def __init__(self, n_input, n_hidden, transfer_function=tf.nn.softplus, optimizer=tf.train.AdamOptimizer(),
        ...
        self.reconstruction = tf.add(tf.matmul(self.hidden, self.weights['w2']), self.weights['b2'])  # 数据复原和重建
    ...
    def _initialize_weights(self):
        ...
        all_weights['w2'] = tf.Variable(
            tf.zeros([self.n_hidden, self.n_input], dtype=tf.float32))  # 因为输出层没有使用激活函数，所以直接初始化为0
        all_weights['b2'] = tf.Variable(tf.zeros([self.n_input], dtype=tf.float32))
        return all_weights

    # 训练一个batch的数据并返回当前的损失cost
